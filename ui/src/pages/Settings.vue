<template>
  ...
  <PageContainer subtitle="主题、默认模型与厂商 API Key（Key 仅缓存在本机）">
    ...
    <Sheet :open="editorOpen" @update:open="editorOpen = $event">
      <SheetContent class="sm:max-w-md" @close="editorOpen = false">
        <SheetHeader>
          <SheetTitle>{{ editorPreset ? `配置 ${editorPreset.name}` : '配置厂商' }}</SheetTitle>
          <SheetDescription>Key 仅保存在本机浏览器</SheetDescription>
        </SheetHeader>
        <FieldGroup v-if="editorPreset" class="px-4">
