...
### 3.13 云函数（GoFunctions，Go Function 控制台 + HTTP 调用）

实现：`internal/api/gofunction_handler.go` + `gofunction/jsonrun.go`（解释执行）。管理面前缀 `:p` = `/v1/projects/:projectID`；调用面独立前缀 `/go/:projectID/:name/:functionName`（须在 SPA fallback 前注册，GET 返回 405 JSON）。计划详见 [`ui-gofunction-plan.md`](./ui-gofunction-plan.md)。

权限：读 `DatabaseRead`，写 `DatabaseWrite`；**invoke 为 `DatabaseRead`**。admin 系统项目（`…-99`）不提供：列表空、写 403。

| Method | Path | 权限 | 成功状态 | 说明 |
|---|---|---|---|---|
| GET | `:p/gofunctions` | DatabaseRead | 200 | `{ "functions":[GoFunction] }`；列表不带 `source` |
| POST | `:p/gofunctions` | DatabaseWrite | 201 | `{ name, source }`；保存前编译校验；同名 409 |
| GET | `:p/gofunctions/:name` | DatabaseRead | 200 | 详情含 `source` |
| PUT | `:p/gofunctions/:name` | DatabaseWrite | 200 | `{ source }`；name 不可改；重算 exports |
| DELETE | `:p/gofunctions/:name` | DatabaseWrite | **204 无 body** | 软删（`archived_at`） |
| POST | `/go/:projectID/:name/:functionName` | DatabaseRead | **200** | 响应体即返回值 JSON（**无 envelope**） |
| GET | `/go/:projectID/:name/:functionName` | DatabaseRead | **405** | `method_not_allowed`，防 SPA fallback 吞掉 |

`GoFunction`（snake_case；`file` 服务端派生 = `name + ".go"`，客户端不回传）：
...
保存约定（服务端强制）：每个包级导出函数必须 `func Name(req T) R`（恰好 1 入参 1 返回）；`ValidateHTTPFuncs` 编译 + 类型预演失败 → 400（message 列出每个不合规函数名与原因）；零导出 → 400；name 不符 `^[A-Za-z][A-Za-z0-9_]{0,62}# SimpleBase 前后端 HTTP 交互协议（proto-http）
...
运行时不可用时流式仍 200：先 `error` 再 `end`。`cancel` 取消 run context；已结束的 run 原样返回。
 ...
 → 400；源码 > 256KiB → 400；每项目 > 100 个 → 422。

invoke：body 为空视为 `{}`（仅 struct/map/slice 入参）；基本类型入参 body 为对应 JSON 字面量（`"abc"` / `42`）。运行沙箱为解释器（非 OS 沙箱），仅已注册标准库可 import；执行超时 10s → 504。

错误码：`invalid_gofunction_name` 400 · `gofunction_compile_error` 400（保存）/500（invoke 时环境变化）· `gofunction_signature_invalid` 400 · `gofunction_source_too_large` 400 · `gofunction_already_exists` 409 · `gofunction_not_found` 404 · `function_not_found` 404（含未导出）· `method_not_allowed` 405 · `gofunction_limit_exceeded` 422 · `gofunction_runtime_error` 500。

Metrics（`sys_metric_samples`，项目维度）：`gofunction_invokes`（计数）· `gofunction_invoke_duration_ms` · `gofunction_invoke_errors` · `gofunction_compile_ms`。

---

### 3.14 定时任务（CronJobs，云函数定时调用）

实现：`internal/api/cronjob_handler.go` + `internal/cronjob/scheduler.go`（tick 30s + CAS 认领）+ `internal/systemdb/cron_jobs.go`（v29/v30）。前缀 `:p` = `/v1/projects/:projectID`。计划详见 [`ui-cronjob-plan.md`](./ui-cronjob-plan.md)。

权限：读 `DatabaseRead`，写 `DatabaseWrite`（含 trigger）。admin 系统项目只读：列表空、写 503。调度器单进程语义；手动 trigger 异步执行不改排期；重启后到期任务补跑一次（at-least-once）。

| Method | Path | 权限 | 成功状态 | 说明 |
|---|---|---|---|---|
| GET | `:p/cron-jobs` | DatabaseRead | 200 | `{ "jobs":[CronJob] }`；附 `target_missing` |
| POST | `:p/cron-jobs` | DatabaseWrite | 201 | 校验：name → schedule → 目标 → input_json；同名 409 |
| GET | `:p/cron-jobs/:jobID` | DatabaseRead | 200 | 详情 |
| PATCH | `:p/cron-jobs/:jobID` | DatabaseWrite | 200 | name 不可改；调度变更重算 `next_run_at` |
| DELETE | `:p/cron-jobs/:jobID` | DatabaseWrite | **204 无 body** | 软删；历史运行记录保留 |
| GET | `:p/cron-jobs/:jobID/runs?limit=` | DatabaseRead | 200 | `{ "runs":[CronJobRun] }` 新→旧，默认 20 上限 100 |
| POST | `:p/cron-jobs/:jobID/trigger` | DatabaseWrite | **202** | `{ "triggered": true }`；执行中再触发 409 |
...
```json
{
  ...
  "enabled": true,
  "last_run_at": "2026-09-19T02:00:00Z",
...
`CronJobRun`：`{ id, job_id, trigger(scheduled/manual), status(running/completed/failed/canceled), error, duration_ms, response_json(截断 4KB), started_at, finished_at, created_at }`。

约定（服务端强制）：name `^[A-Za-z][A-Za-z0-9_-]{0,62}# SimpleBase 前后端 HTTP 交互协议（proto-http）
...
；`schedule_kind=cron` 时 `cron_expr` 必须可解析（5 字段 UTC，分钟粒度，`internal/crontab`）；`schedule_kind=interval` 时 `interval_seconds` ∈ [60, 2592000]；模式互斥（另一字段清空）。目标必须为项目内已存在云函数的已导出函数；`input_json` 合法 JSON ≤ 8KB（保存不做类型级 dry-run，不匹配在执行时记 failed）。时间一律 UTC；`next_run_at` 服务端计算。

错误码：`invalid_cron_job_name` 400 · `invalid_cron_expression` 400 · `invalid_interval` 400 · `invalid_request` 400（body/input_json）· `gofunction_not_found` 400（目标文件不存在）· `function_not_exported` 400 · `cron_job_not_found` 404 · `cron_job_exists` 409 · `cron_job_running` 409（trigger 冲突）· `cron_scheduler_unavailable` 503 · `readonly_mode` 503。

Audit：create/update/delete/trigger 记 `kind=cronjob`（detail 只含动作与任务名）。调度器日志走 `sys_log_events`（logger=`cronjob`）。
...
| GoFunctions | gofunctions CRUD / invoke POST / GET 405 演示 |
...
services/types.ts 的 Api 接口域        对应章节    状态
  ...
  gofunctions.* → §3.13 ✔ 云函数 CRUD；invoke 走独立 /go 前缀
```
