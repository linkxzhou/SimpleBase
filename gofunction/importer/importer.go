...
type Importer struct {
	...
	externalObjects map[string]*Object
}
...
func NewImporter(importPackage ...*ssa.Package) *Importer {
	i := &Importer{
		...
		externalObjects: make(map[string]*Object),
...
// NewImporterWithRegistry 创建带自定义注册器的包导入器
func NewImporterWithRegistry(registry *Registry, importPackage ...*ssa.Package) *Importer {
	importer := NewImporter(importPackage...)
...
func (p *Importer) Import(path string) (*types.Package, error) {
	...
	if pkg := p.packageCache[path]; pkg != nil {
		return pkg, nil
	}
...
func (p *Importer) newObject(pkg *types.Package, obj *Object) (object types.Object) {
	name := obj.Name
	switch obj.Kind {
	case TypeKind:
		typ := p.typeOf(obj.Type, pkg)
	...
	case ConstantKind:
		...
		case reflect.Complex64, reflect.Complex128:
			// TODO 目前暂未对Complex做支持
		}
		object = types.NewConst(token.NoPos, pkg, name, p.typeOf(obj.Type, pkg), constValue)
		pkg.Scope().Insert(object)
	case FunctionKind, BuiltinFunctionKind:
		typ := p.typeOf(obj.Type, pkg)
...
func (p *Importer) Package(path string) *types.Package {
	...
	if pkg == nil {
		if externalPkg, exists := p.registry.GetPackage(path); exists {
...
// SsaPackage 根据包名返回经过gofun编译的包
func (p *Importer) SsaPackage(name string) *ssa.Package {
	return p.ssaPackages[name]
}

// ExternalObjects 返回已加载的外部对象映射（key 为 "pkgPath.ObjectName"）
func (p *Importer) ExternalObjects() map[string]*Object {
	return p.externalObjects
}
