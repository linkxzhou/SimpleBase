...
type Program struct {
	...
	// constCache *ssa.Const -> value.Value 求值缓存（SSA 常量为稳定单例，值恒定）。
	// 仅缓存基础类型；复合零值保持每次新建，避免共享可变结构。
	// 并发 Run 同一 Program 时懒填充，须并发安全。
	constCache sync.Map
	// funcCache *ssa.Function -> value.Value 包装缓存（makeFunc 反射包装昂贵且幂等）
	funcCache sync.Map
	// blockCache *ssa.BasicBlock -> *compiledBlock 预编译指令分发表（P2-1）。
	// BuildProgram 后预热，运行期只读；漏网块由 getCompiledBlock 惰性编译。
	blockCache sync.Map
	// layoutCache *ssa.Function -> *funcLayout 槽位布局（P3-1）。
	// SSA 构建后函数结构不变，布局不可变，并发 Run 只读共享。
...
func BuildProgram(seqid, fname, sourceCode string, packages ...*ssa.Package) (*Program, error) {
	...
	// 预编译全部基本块（P2-1）：消除运行期类型 switch 分发开销
...
func (p *Program) RunWithContext(seqid, funcName string, params ...interface{}) (result interface{}, ctx *Context, err error) {
	ctx = newCallContext()
	result, err = p.runWithContext(seqid, funcName, ctx, params...)
	return result, ctx, err
}

// runWithContext 在给定上下文中执行函数。
// 拆分自 RunWithContext：Executor 需要先创建 ctx 注册取消函数
// （超时定时器经 Interrupt 真正取消执行，见 P2-4），再进入执行。
func (p *Program) runWithContext(seqid, funcName string, ctx *Context, params ...interface{}) (result interface{}, err error) {
	defer func() {
