...
func (r *sqlRepository) Enqueue(ctx context.Context, job Job) error {
	if job.ID == "" {
	...
	var existing string
	err := r.db.QueryRowContext(ctx, `SELECT id FROM sys_jobs WHERE id = ?`, job.ID).Scan(&existing)
	if err == nil {
	...
	_, err = r.db.ExecContext(ctx,
		`INSERT INTO sys_jobs(id, operation_id, database_id, project_id, type, status, attempt, max_attempts,
			run_after, payload_json, last_error, created_at, updated_at)
		 VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		job.ID, job.OperationID, job.DatabaseID, job.ProjectID,
		string(job.Type), string(job.Status), int64(job.Attempt), int64(job.MaxAttempts),
		job.RunAfter.UTC(), job.PayloadJSON, job.LastError, job.CreatedAt.UTC(), job.UpdatedAt.UTC())
	if err != nil {
		if isUniqueViolation(err) {
			return fmt.Errorf("%w: job %s", ErrAlreadyExists, job.ID)
		}
...
func (r *sqlRepository) Claim(ctx context.Context, workerID string, now time.Time) (Job, error) {
	...
	err = tx.QueryRowContext(ctx,
		`SELECT id, operation_id, database_id, project_id, type, status, attempt, max_attempts,
		 ...
		 LIMIT 1`,
		string(JobStatusPending), now.UTC()).Scan(
		&j.ID, &j.OperationID, &j.DatabaseID, &j.ProjectID,
		&typ, &status, &attempt, &maxAttempts,
	...
	_, err = tx.ExecContext(ctx,
		`UPDATE sys_jobs SET status = ?, attempt = ?, updated_at = ? WHERE id = ? AND status = ?`,
		string(JobStatusRunning), attempt+1, now.UTC(), j.ID, string(JobStatusPending))
	if err != nil {
		return Job{}, err
	}
...
func (r *sqlRepository) Complete(ctx context.Context, id string) error {
	now := time.Now().UTC()
	var status string
	err := r.db.QueryRowContext(ctx, `SELECT status FROM sys_jobs WHERE id = ?`, id).Scan(&status)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return ErrNotFound
		}
	...
	_, err = r.db.ExecContext(ctx,
		`UPDATE sys_jobs SET status = ?, updated_at = ? WHERE id = ? AND status = ?`,
...
func (r *sqlRepository) Retry(ctx context.Context, id string, lastErr string, next time.Time) error {
	now := time.Now().UTC()
	var attempt, maxAttempts int64
	err := r.db.QueryRowContext(ctx,
		`SELECT attempt, max_attempts FROM sys_jobs WHERE id = ?`, id).Scan(&attempt, &maxAttempts)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return ErrNotFound
		}
	...
	res, err := r.db.ExecContext(ctx,
		`UPDATE sys_jobs SET status = ?, last_error = ?, run_after = ?, updated_at = ? WHERE id = ? AND status = ?`,
		newStatus, lastErr, next.UTC(), now, id, string(JobStatusRunning))
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
		var status string
		if scanErr := r.db.QueryRowContext(ctx, `SELECT status FROM sys_jobs WHERE id = ?`, id).Scan(&status); scanErr != nil {
...
func (r *sqlRepository) GetJob(ctx context.Context, id string) (Job, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, operation_id, database_id, project_id, type, status, attempt, max_attempts,
			run_after, payload_json, last_error, created_at, updated_at
		 FROM sys_jobs WHERE id = ?`, id).Scan(
		&j.ID, &j.OperationID, &j.DatabaseID, &j.ProjectID,
		&typ, &status, &attempt, &maxAttempts,
		&j.RunAfter, &j.PayloadJSON, &j.LastError, &j.CreatedAt, &j.UpdatedAt)
	if err != nil {
...
func (r *sqlRepository) ListJobs(ctx context.Context, databaseID string, limit int) ([]Job, error) {
	if limit <= 0 || limit > 200 {
		limit = 50
	}
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, operation_id, database_id, project_id, type, status, attempt, max_attempts,
			run_after, payload_json, last_error, created_at, updated_at
		 FROM sys_jobs WHERE database_id = ?
		 ORDER BY created_at DESC LIMIT ?`, databaseID, int64(limit))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var typ, status string
		var attempt, maxAttempts int64
		if err := rows.Scan(&j.ID, &j.OperationID, &j.DatabaseID, &j.ProjectID,
			&typ, &status, &attempt, &maxAttempts, &j.RunAfter,
			&j.PayloadJSON, &j.LastError, &j.CreatedAt, &j.UpdatedAt); err != nil {
			return nil, err
		}
		j.Type = JobType(typ)
		j.Status = JobStatus(status)
